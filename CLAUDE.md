...
- `src/components/CompaniesAttending.astro` - "Ruby companies attending" on the homepage: every company with two or more conference ticket holders, with the Cloud Card characters those people have published on the Pier. Data is the snapshot `src/content/attending-companies.json`, written by `scripts/fetch-attending-companies.js` (part of `npm run fetch-data`): it groups Luma guests by their company answer (organizer tickets and Evil Martians excluded, spellings normalized), then matches characters from `clouds.sfruby.com/api/characters` by name and by company. The snapshot carries company names, counts, and only the names and images already public on the Pier. Refresh it with `node scripts/fetch-attending-companies.js` (needs `LUMA_API_KEY`) and commit the JSON so `build:dev` stays current.
