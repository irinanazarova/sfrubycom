...
- `src/pages/speakers.astro` - The 2026 lineup in full: a character-select jump index, then one profile card per speaker. Merges two sources on `speakerKey` (`src/utils/cfpTalks.js`): `src/data/conference-2026.js` owns who is confirmed plus the sprite and bio, the CFP feed owns the talk title and abstract. Titles/abstracts are baked in from the `src/content/cfp-talks.json` snapshot and refreshed client-side from the live API, the same contract `SpeakerRoster.astro` uses on the homepage. The 2025 lineup lives on unchanged at `/speakers-2025` (`speakers-2025.astro`), which `/schedule-2025` and `getSpeakerOGMetadata` deep-link into. The 2025 schedule moved alongside it: `schedule-2025.astro`. The 2026 schedule is a homepage section, `src/components/Schedule.astro` (`/#schedule`; `/schedule` and, since September 2026, `/speakers` 302 there with `force = true`, since the talk cards carry everything the speakers page did; the page still builds but nothing links to it), rendered from `src/data/schedule-2026.js`: one stage, two audiences (`startups` and `household`, labelled "Small teams" and "Big teams" since September 2026; the ids did not change), each talks block tagged with the audience in the hall and carrying the conversation `group` (title, `hosts`, or `hostTbc`) the other audience is in at the same time; keynotes are `audience: "all"`. **The CFP app owns talk titles.** A `title` in `schedule-2026.js` is a placeholder, transcribed from the organizers' sheet, that holds the slot while a speaker's CFP entry still says TBD; a real CFP title always replaces it (`resolveTitle` in `src/utils/speakerProfiles.js`). So a wrong title is fixed in the CFP app, not here, and editing this file changes nothing for a speaker who has already published one. The rule is applied in one place and read everywhere: the slot button, the card it opens and the Markdown twin all print `profile.title`, so no surface can name a talk differently from another. Group titles have no CFP source and do live in the file. Edit the sheet, then this file; the networking section's group list and the schedule both read from it. The card a slot opens leads with the speaker's animated share card, full width across the top, then the talk's day and time (the row you tapped has it, and the card covers the row), the text of the talk, the bio, and a button that downloads the card. The text under the card repeats the title and abstract on purpose: at phone width the card's own type is far too small to read. The card's src is withheld until the dialog opens (`loadCard` in `Schedule.astro`), because every scheduled speaker has a dialog in the homepage's DOM and 20 eager GIFs would be 8MB on first paint. The close button gets a bar of its own above the card rather than floating over it: the share card's title starts in its top-left corner and its Keynote plate sits in the top-right, so there is no empty corner to close from. Every talk slot is a button that opens the speaker's full card in a native `<dialog>` (`?speaker=<key>` in the URL, so a slot is shareable; a full-width bottom sheet on phones, a two-column card from `md`); the card is `src/components/SpeakerProfile.astro`, the same component `/speakers` renders, so the two can never disagree about a speaker.
- `src/pages/index.astro` - The conference homepage, ordered hero → `Schedule` → `TicketsBlock` (the three live tiers as cards plus the manager page as a same-weight CTA; placements `tickets-company`, `tickets`, `tickets-day`) → sponsors → `CompaniesAttending` → `Pier` → venue → `GettingThere` → `Crew` → community. The closing "Tickets are on sale" block was replaced by `Crew.astro` in September 2026 (the tickets block after the schedule carries the ask): organizers and volunteers together on one island, from `src/content/crew-2026.json`, which `scripts/fetch-attending-companies.js` writes from the Luma guests on the "Organizer/volunteer" ticket (split by company in the JSON, Evil Martians as organizers, though the island shows them together), with the same character snapshot as the companies. Everyone is named under the island; only people with a Cloud Card stand on it, on purpose: the organizers nudge the rest to draw one, and each climbs on at the next daily build. `src/data/venue-2026.js` holds the address, the transit lines and the two SFMTA garages with their weekday rates (rendered by `GettingThere.astro` and by the homepage's Markdown twin), checked against sfmta.com in September 2026; the 21 Hayes and 47 Van Ness are suspended in 2026, so do not add them back from older guides, and recheck the garage rate tables before the conference. `SpeakerRoster`, `WhatYouHear`, `ConferenceNetworking`, `TicketLadder` and the closing `IslandNav` were taken off the homepage in September 2026: the schedule section carries the speakers, talks, groups and both ticket buttons, so they repeated it. The first four are now rendered by no page; they are kept in the repo (SpeakerRoster holds the CFP refresh pattern) and can be deleted once the new homepage has settled. **The conference is the San Francisco Ruby Conference, without "Startup", since September 2026**: large sponsors read "Startup Conference" as no hiring value for them, so the positioning is two audiences served on purpose, the startups building on Ruby and the household names running it at scale, with Day 1 AI-heavy and Day 2 AI-light (the days were "AI on Rails" and "The rest of the stack" until mid-September 2026). The hero statement is "Let's bring programmer delight and Ruby into the future", in gold over the sky; the two-audience positioning carries in the meta description, the schedule lede and the section headings. Keep them on that line, and keep "Startup Conference" out of new copy (the 2025 archive pages and `meetup-2026-06.astro` keep the old name as history). The homepage roster is a 4-speaker teaser whose CTA carries the rest to `/speakers`. **The hero stops short of the fold on laptops on purpose** (`lg:min-h-[calc(100svh-10rem)]`, tighter `lg:` paddings, the fighter grid at 7 x 3 with 21 cells, the schedule section's `lg:pt-12`): at 1440x900 the "The schedule" heading is visible under the ticker without scrolling, which is what tells a first-time visitor the page continues. Re-measure `#schedule h2` after touching the hero; a taller hero quietly undoes it. `public/schedule_live_dragon.png` is the early-bird dragon (`early_bird_dragon.png`, still the source art) with its sign repainted to "Schedule is live!" in Press Start 2P and snapped to the sign's two colors; it links to `#schedule`, top-right of the title art on desktop and in the title stack under the CTA on phones. To change the sign text again, wipe the text box (386x168 at 352,366, fill `#E1BC8C`), set the new lines at 40px, and remap the box to `#E1BC8C` and `#2D1111`.
...
- `Accept: text/markdown` on the HTML URL returns the twin (`netlify/edge-functions/markdown-negotiation.ts`; Netlify only, `astro preview` has no edge, so check it on a deploy preview or after the deploy).

Every page with a twin carries `<link rel="alternate" type="text/markdown">`, added by the page shells from `markdownTwinFor()`.

**Which routes have a twin is decided in `src/lib/markdown-twins.js`; how each is rendered is in `src/lib/llm-content.js`.** To add one: add the route to `TWINS` and a renderer to `RENDERERS` (the build fails on a twin without one). The page shells and the edge function both read `TWINS`, so nothing else is registered by hand. Content the twins share with a page lives in a data module, never inline in the `.astro` file: `src/data/venue-2026.js` (address, transit, garages), `src/data/sponsor-tiers-2026.js` (tiers, stats, reasons), `src/data/manager-email-2026.js` (the approval email). Ticket links in the twins go through `ticketLink("<slug>-md")`, so a purchase that started in an LLM answer shows up in Plausible and Luma like any other placement.
...
### Speaker share cards (`public/speaker-card/*.gif`)

One animated 1280x720 card per speaker, for them to post when they announce their talk: the talk title, when it is on, the abstract, and their pixel character on an island under drifting clouds. JP Camara made the first one by hand and posted it; this generates the rest from the same data the site renders.

Rebuild with **`npm run speaker-cards`** (add speaker keys to do just those, `--png` to also keep a still frame). Needs ffmpeg and the chromium Playwright downloads (`npx playwright install chromium`). It is not part of `npm run build`: a production build would have to download chromium first, and the cards change only when a title does.

- **Content is `buildProfiles()`**, the merge `/speakers` and the schedule card already read, so a card can never name a talk differently from the site. The CFP app owns the titles here too: after a speaker publishes or edits one, `npm run fetch-data` then `npm run speaker-cards`. A speaker with no CFP abstract gets their roster bio.
- **`scripts/speaker-gif-template.html` is the frame.** The generator serves it over a static server rooted at `public/`, so its font and sprite URLs are the production ones, then steps `window.setFrame(i)` and screenshots each frame. Nothing animates on a timer, so an unchanged speaker rebuilds to the same bytes.
- **Every size on the card is measured, after `document.fonts.ready`.** Press Start 2P measures nothing like the fallback face, and fitting before it loads put nine-line titles on the long cards.
- **All motion stays in the right third**, clear of the text column. ffmpeg stores only the rectangle that changed between frames, so a still left column is the difference between a 400KB card and a 1.6MB one. Keep it that way when editing: a drifting cloud behind the title costs four times the file size and covers the words.
- **Sprites are sized by height, never width.** They share a height (240px inside a 256 box) but run from 118px to 204px wide, so sizing on width draws one speaker twice the size of another.
- The download button on the card appears only when the file is on disk (`src/utils/speakerCard.js`), so a speaker added since the last run has no broken link.
